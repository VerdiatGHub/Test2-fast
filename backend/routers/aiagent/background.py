...
def next_step(tid: str, next_step_req: BackgroundNextStepRequest, db: Session = Depends(get_session),
    ...
    response_data = None
    model_type = os.getenv("COMPUTER_USE_AGENT_MODEL_TYPE")
    if task.extended_thinking_mode is True and model_type not in ["openrouter", "9router"]:
        ...
        # OpenRouter returns content as a string, even if thinking is enabled on their end
