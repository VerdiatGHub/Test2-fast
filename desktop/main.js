...
ipcMain.handle('get-suggestions', async (_, baseURL) => {
  return new Promise((resolve, reject) => {
    ...
    const suggestor = isDev
      ? spawn(isWindows ? './aiagent/venv/Scripts/python' : './aiagent/venv/bin/python', ['./aiagent/suggestor.py'], {
          env: {
            NEURALAGENT_API_URL: baseURL,
            NEURALAGENT_USER_ACCESS_TOKEN: store.get(constants.ACCESS_TOKEN_STORE_KEY),
            PYTHONUTF8: '1',
      ...
      : spawn(path.join(process.resourcesPath, isWindows ? 'suggestor.exe' : 'suggestor'), [], {
          env: {
            NEURALAGENT_API_URL: baseURL,
            NEURALAGENT_USER_ACCESS_TOKEN: store.get(constants.ACCESS_TOKEN_STORE_KEY),
          },
    ...
    suggestor.on('error', (err) => {
      console.error('Suggestor spawn error:', err);
      resolve([]);
    });
